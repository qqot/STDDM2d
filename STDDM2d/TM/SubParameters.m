...
PMLnx=8;
PMLny=8;
subx=Subdomain.subx;                 % [xmin,xmax] in the subdomain
suby=Subdomain.suby;                 % [ymin,ymax] in the subdomain
Totalsubnx= Subdomain.Totalsubnx;    % Number of grids in the subdomain (x direction)
Totalsubny= Subdomain.Totalsubny;    % Number of grids in the subdomain (y direction)
Subdomain.global_index=zeros(1,Totalsubnx*Totalsubny);   % Map of subdomain index and global index
...
%% PML epsilon
% PML stretching treatment for cutting off the dielectric
for ii=1:8
    Subdomain.eps(ii,:)=Subdomain.eps(9,:);
    Subdomain.eps(Totalsubnx-ii+1,:)=Subdomain.eps(Totalsubnx-8,:);
    Subdomain.eps(:,ii)=Subdomain.eps(:,9);
    Subdomain.eps(:,Totalsubny-ii+1)=Subdomain.eps(:,Totalsubny-8);
end
%% T matrix
